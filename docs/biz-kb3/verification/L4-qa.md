...
- 生成时间: 2026-09-17T14:02:49.930976+00:00
- 知识库: `D:\openCodeProject\code-to-doc\docs\biz-kb3`
...
- **answer_accuracy: 56.2%**
- **faithfulness: 98.8%**
- **retrieval_hit_rate: 89.1%**
- **refusal_correctness: 90.5%**
- answer_score_weighted: 72.7%
...
| KB-Q007 | positive | correct | 是 | 是 | 一次性固定费用（FIXED 项目）会被按比例折算吗？ |
...
| KB-Q011 | positive | partial | 否 | 是 | 什么叫信用余额调整（CBA，credit balance adjustment）？ |
...
| KB-Q018 | positive | correct | 是 | 是 | 管理员给账户加一笔 20 美元的信用（credit），在发票层面会怎么体现？ |
| KB-Q019 | positive | wrong | 是 | 是 | 系统默认会在发票生成后自动扣款吗？什么情况下不会自动扣款？ |
| KB-Q020 | positive | wrong | 是 | 是 | 可以用一笔支付同时支付多张发票吗？ |
| KB-Q021 | positive | correct | 是 | 是 | 发票支付发生后如果退款（refund），在发票-支付记录里如何体现？ |
| KB-Q022 | positive | correct | 是 | 是 | 支付插件返回 ERROR（例如余额不足）时，Kill Bill 会把交易置为哪个状态、返回什么 HTTP 码？ |
...
| KB-Q030 | positive | wrong | 否 | 是 | Kill Bill 核心系统会在发票上自动计算并添加税额（TAX）吗？ |
| KB-Q031 | positive | wrong | 是 | 是 | Kill Bill 中权限（Permissions）、角色（User Role）和用户（User）是什么关系？ |
...
| KB-Q042 | positive | correct | 是 | 是 | 支付插件返回 CANCELED（例如网关根本没被调用、插件硬失败）时，交易状态、支付状态和 HTTP 返回码分别是什么？系统会尝试修复吗？ |
| KB-Q043 | positive | correct | 是 | 是 | 如果一笔支付被 control plugin 中止（abort），系统会记录下什么？返回什么 HTTP 码？ |
| KB-Q044 | positive | partial | 是 | 否 | 一笔支付失败并重试时，会产生几个 payment transaction、几个 payment attempt？重试后这些记录分别处于什么状态？ |
| KB-Q045 | positive | correct | 否 | 是 | Kill Bill 默认的失败支付重试日程是什么？可以怎么改？ |
| KB-Q046 | positive | partial | 是 | 是 | 支付重试系统（Payment Retry）和逾期系统（Overdue）是一回事吗？它们怎么配合？ |
| KB-Q047 | positive | correct | 是 | 是 | 客户发起拒付（chargeback）后，发票-支付记录里会多出什么类型的记录，金额符号是怎样的？ |
| KB-Q048 | positive | correct | 是 | 是 | 账户的默认支付方式在收款时怎么被使用？能不能临时改用别的支付方式扣款？ |
| KB-Q049 | positive | partial | 是 | 是 | Kill Bill 通过什么机制在支付执行前后修改支付行为（比如中止支付、改金额/币种、改支付方式、安排重试）？怎么启用它？ |
| KB-Q050 | positive | partial | 是 | 是 | 写 overdue.xml 时，状态定义的排列顺序有要求吗？状态名有什么限制？ |
| KB-Q051 | positive | partial | 是 | 是 | overdue 配置里 initialReevaluationInterval 和每个状态里的 autoReevaluationInterval 分别是什么？ |
| KB-Q052 | positive | partial | 否 | 是 | 账户打了 OVERDUE_ENFORCEMENT_OFF 控制标签会怎样？移除标签后呢？ |
...
| KB-Q056 | positive | partial | 是 | 是 | 目录里 plan/phase 的 name 有什么命名限制？name 和 prettyName 有什么区别？ |
| KB-Q057 | positive | partial | 是 | 是 | 附加组件（ADD_ON）通常使用哪种计费对齐方式？如果 BCD 落在当月不存在的日期（比如 4 月 31 日）会怎么处理？ |
| KB-Q058 | positive | partial | 是 | 是 | 目录规则里计划变更时机（changePolicy）可以取哪些值？ILLEGAL 表示什么？ |
| KB-Q059 | positive | wrong | 是 | 是 | 新增附加组件时，怎样让它和基础计划共用一个试用期结束时间？ |
| KB-Q060 | positive | partial | 是 | 是 | 订阅取消时，entitlement（权限）和 billing（计费）为什么可能不一致？CTD 是什么？ |
| KB-Q061 | positive | correct | 是 | 是 | 用量计费支持哪些 billingMode？预付费（IN_ADVANCE）现在支持吗？ |
| KB-Q062 | positive | correct | 是 | 是 | CAPACITY 用量模式下，多个 unit 的峰值是怎么决定用哪一档价格的？它和 CONSUMABLE 的本质区别是什么？ |
| KB-Q063 | positive | correct | 是 | 是 | 用量阶梯（tier）定义里，max 设为 -1 代表什么？tieredBlock 里的 size 又是什么意思？ |
| KB-Q064 | positive | correct | 是 | 是 | 用量发票默认是每个阶梯输出一条 USAGE 项目，还是汇总成一条？如果要按明细输出，怎么开关？ |
| KB-N010 | negative | other | - | 是 | 能不能让同一张发票里同时包含美元和欧元两种货币的费用项目？ |
...
| KB-N017 | negative | other | - | 是 | 能不能在 Kill Bill 里发起并跟踪一起来自支付网关的拒付争议（dispute）处理流程？ |
